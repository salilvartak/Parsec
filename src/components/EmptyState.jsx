  ...
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'center', padding: isMobile ? '22px 16px 40px' : 40, overflowY: 'auto' }} onDragOver={e => e.preventDefault()} onDrop={onDrop}>
      <div style={{ width: '100%', maxWidth: 460, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: isMobile ? 16 : 22, textAlign: 'center' }}>
        ...
        <textarea placeholder="Paste JSON here…" spellCheck={false} autoCapitalize="off" autoCorrect="off"
          onChange={e => { if (e.target.value.trim()) loadFromText(e.target.value, 'pasted.json'); }}
          style={{ width: '100%', height: isMobile ? 140 : 120, resize: 'none', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface)', color: 'var(--text)', fontFamily: "'JetBrains Mono',monospace", fontSize: 12.5, padding: 12 }} />
        {/* upload and fetch sit side by side on a desktop, stacked on a phone */}
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 10, width: '100%' }}>
          ...
          <button onClick={() => fileRef.current?.click()} style={{ flex: 1, height: isMobile ? 42 : 36, borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', font: "500 12.5px 'Inter',sans-serif", cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ flex: 1.4, minWidth: 0, height: isMobile ? 42 : 36, display: 'flex', border: '1px solid var(--border)', borderRadius: 7, background: 'var(--surface)', overflow: 'hidden' }}>
            ...
            <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchUrl()}
              type="url" inputMode="url" autoCapitalize="off" autoCorrect="off" spellCheck={false}
              placeholder="https://api.example.com/data" style={{ flex: 1, minWidth: 0, border: 'none', background: 'transparent', color: 'var(--text)', font: "400 12px 'Inter',sans-serif", padding: '0 8px', outline: 'none' }} />
