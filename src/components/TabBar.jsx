      ...
      {documents.map(doc => {
        ...
        return (
          <div key={doc.id}
            ...
            style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '0 8px 0 10px', cursor: editing ? 'text' : 'pointer',
            ...
            {!editing && (
              <button onClick={e => { e.stopPropagation(); beginRename(doc); }} title="Rename tab"
                style={{ border: 'none', background: 'transparent', color: 'var(--text3)', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}>
                <svg width="11" height="11" viewBox="0 0 12 12"><path d="M8.5 1.5l2 2L4 10l-2.5.5L2 8 8.5 1.5z" fill="none" stroke="currentColor" strokeWidth="1.1" strokeLinejoin="round" /></svg>
              </button>
            )}
